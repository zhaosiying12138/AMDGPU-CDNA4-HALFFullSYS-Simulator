...
class CodeObjectSpecTest(unittest.TestCase):
    ...
    def test_isa_audit_is_explicitly_blocked(self) -> None:
        ...
        execution = fixtures["gfx950-rdc-gpuReadWrite-v1"]["functional_execution"]
        self.assertEqual(execution["status"], "accepted-locked-functional-case")
        self.assertEqual(execution["checkpoint"], "CP-0020")
        self.assertEqual(execution["evidence_manifest"], "EV-0049")
        self.assertEqual(execution["evidence_manifest_sha256"], "80f81de192cc73e7e9e4e4d6aff0554c69e9f5df5fbe273b137fb97e10d2f270")
        self.assertEqual(execution["detailed_evidence_sha256"], "e8989d2bfb3ba6f3de6b1f33f6a5c218385785e7a18387080469ceca9cecdc8e")
        self.assertEqual(execution["gem5_child_head"], "d4b96099040140369b9d55108f7bde21e0acfd59")
        self.assertEqual(execution["gem5_child_tree"], "95acf2260a9a9c3b68761fa0695b1350321516")
        self.assertEqual(execution["geometry"], {"grid_size_x": 1024, "workgroup_size_x": 256, "workgroup_count": 4, "wavefront_size": 64, "wave_count": 16})
        starts = execution["instruction_start_contract"]
        self.assertEqual((starts["pc_count_per_wave"], starts["total_count"], starts["cu_stat_value"], starts["completed_waves"]), (19, 304, 304, 16))
        self.assertEqual(execution["oracle"], {"separate_4k_buffers": True, "elements": 1024, "a": "unchanged", "b": "gid", "c": "a"})
        self.assertEqual(execution["completion"]["mqd_read_index_before"], 0)
        self.assertEqual(execution["completion"]["mqd_read_index_after"], 1)
        self.assertEqual(execution["completion"]["signal_layout"], "direct_u64")
        self.assertEqual(execution["completion"]["signal_before"], 1)
        self.assertEqual(execution["completion"]["signal_after"], 0)
        self.assertTrue(execution["completion"]["packet_retired"])
        self.assertTrue(execution["completion"]["pins_released"])
        self.assertEqual(execution["runtime_graph"]["cpu_objects"], 0)
        self.assertEqual(self.spec["execution_addendum"]["status"], "one-locked-functional-case-accepted-general-gfx950-blocked")
        triton = self.spec["triton_compile_provenance_addendum"]
        self.assertEqual(triton["checkpoint"], "CP-0021")
        self.assertEqual(triton["evidence_manifest"], "EV-0051")
        self.assertEqual(triton["evidence_manifest_sha256"], "d3ca97990298b3ffc68194341495028d16b3ab981f5966d3a7e479614a0bb158")
        self.assertEqual(triton["detailed_evidence_sha256"], "d29cb0f1f7ea505391b24083c428d832b704a2fb8642ad576a2e4fd94e026d39")
        self.assertEqual(triton["runtime_child_head"], "386713738a9eb050394e74152fe5acaf16435abe")
        self.assertEqual(triton["runtime_child_tree"], "0011ed99bc09baaa06f57887141217d56c24626d")
        self.assertEqual(triton["inputs"]["tutorial"]["sha256"], "842430949e0ccde4fbce07606cce3ac4bac36bf21b2b12619a31b795ca4029b3")
        self.assertEqual(triton["inputs"]["hsaco"]["sha256"], "ee8b0f892da7ab1886f17ee66f88de5c23e05a48f7f361e02bd0707c9a11826e")
        self.assertEqual(triton["metadata"]["target"], "amdgcn-amd-amdhsa-unknown-gfx950")
        self.assertEqual(triton["metadata"]["descriptor_kernarg_preload_dwords"], 12)
        self.assertEqual(triton["metadata"]["descriptor_kernarg_preload_bytes"], 48)
        self.assertFalse(triton["metadata"]["isa_supported_by_gemsim"])
        self.assertEqual(triton["validation"]["runtime_ctest"], "16/16")
        self.assertEqual(triton["validation"]["focused_code_object_ctest"], "4/4")
        for field in ("compiler_invoked", "jit", "launcher", "transport", "execution", "fallback"):
            self.assertFalse(triton["validation"][field])
        self.assertFalse(triton["public_transport_boundary"]["generic_wire_v2"])
        self.assertEqual(triton["public_transport_boundary"]["mapping_va"], 0)
    ...
    def test_document_cross_reference_and_no_forbidden_upload_api(self) -> None:
        ...
        self.assertIn("CP-0020", docs)
        self.assertIn("19-PC", docs)
        self.assertIn("CP-0021", docs)
        self.assertIn("unknown-gfx950", docs)
        self.assertIn("12 DWORD", docs)
