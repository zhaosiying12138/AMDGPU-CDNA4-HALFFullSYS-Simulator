...
- Current root commit before the next checkpoint commit: `47684d91779b01e374c78a5d7d60fdb446b7a723` (bundle/manager)
...
- Bundle/manager/service Python checks pass (`21` bundle/manager/service tests
...
- The kernel build is still running in the isolated
  `build/agentenv-kernel` tree. Staging is allowed; activation is not.
...
Wait for the feature-local kernel build, run `tools/build_agentenv_wsl_kernel.sh stage`,
then commit the source/tool/doc checkpoint. Do not edit `.wslconfig`, invoke
`wsl --shutdown`, start the service, or launch a sandbox until the user approves
the maintenance gate after seeing the exact candidate diff and rollback.
