#!/usr/bin/env python3
"""Run unchanged upstream Triton AMD kernels on runner-owned gem5."""
...
import upstream_triton_runtime_acceptance as contract  # noqa: E402
...
def worker_argv(identity: Mapping[str, Any]) -> list[str]:
    return contract.expected_worker_argv(identity)
    ...
    environment.update(
        {
            ...
            "GEMSIM_RUN_TRITON_CACHE_DIR": str(execution_root / "triton-cache"),
        }
    )
    return environment
...
def run_gate(
    ...
    *,
    ...
    session = lifecycle.run_session(
        ...
        execution_prefix="gs-upstream-triton-",
        ...
        worker_label="upstream Triton AMD worker",
    ...
    try:
        ...
        failure = failure or f"Triton postflight identity failed: {error}"

    cache_paths: tuple[str, ...] = ()
    try:
        cache_paths = contract.cache_artifact_paths(session.execution_root)
    except Exception as error:
        failure = failure or f"Triton JIT cache validation failed: {error}"
    required_artifacts = all(
        (session.execution_root / relative).is_file()
        for relative in contract.CORE_ARTIFACTS
    )
    ...
    success = (
        ...
        and bool(cache_paths)
        and identity_unchanged
    )
    if not success and failure is None:
        if not identity_unchanged:
            failure = "Triton execution identity drifted"
        elif not required_artifacts or not cache_paths:
            failure = "Triton execution artifacts are incomplete"
        else:
            failure = "Triton execution did not meet its contract"

    manifest_core = {
        "schema": contract.RUN_SCHEMA,
        "status": "success" if success else "failed",
        "claim_scope": contract.CLAIM_SCOPE,
        "ordinary_upstream_triton_amd_executed": success,
    ...
    try:
        return lifecycle.publish_source(
            ...
            artifact_paths=contract.CORE_ARTIFACTS + cache_paths,
...
def main() -> int:
    ...
    try:
        ...
        print(f"upstream Triton AMD run failed: {error}", file=sys.stderr)
        return 1
