...
def main() -> int:
    ...
    # SGLang reserves five request slots for the hybrid Mamba state pool when
    # computing max_req_input_len (tp_worker.get_worker_info).  Account for
    # that reservation here; otherwise a 19-token text prompt is rejected by
    # a nominal 20-token context before the model executes.
    # validate_input_length uses ``>=`` rather than ``>``, so leave one extra
    # request slot beyond the exact arithmetic boundary as well.
    capacity = required_tokens + args.max_mamba_cache_size + 1
    context_length = max(args.context_length, capacity)
    max_total_tokens = max(args.max_total_tokens, capacity)
