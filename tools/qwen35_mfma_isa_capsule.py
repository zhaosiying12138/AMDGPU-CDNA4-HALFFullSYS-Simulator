...
the expected result is layout-independent:
  ...
  every MFMA element after the chained second op = 32.0f (A=B=1.0 bf16, K=16,
...
def main() -> int:
    ...
    check(
        library.hipModuleGetFunction(ctypes.byref(function), module, b"_Z14mfma_agpr_testPf"),
    ...
    expectations = (32.0, 32.0, 100.0, 3.0)
    per_slot_bad = [0, 0, 0, 0]
    examples = []
    for lane in range(64):
        for slot in range(4):
            got = words[lane * 4 + slot]
