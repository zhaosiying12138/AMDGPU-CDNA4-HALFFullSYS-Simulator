...
def main() -> int:
    ...
    image = (Path(__file__).parent / "mfma1632_probe.hsaco").read_bytes()
    hip_path = None
    ...
    buf = torch.zeros(4096, dtype=torch.float32, device="cuda")
    ...
    result = {
        ...
        "agpr_all_32": all(v == 32.0 for v in ag),
        "sha256": hashlib.sha256(struct.pack("<128f", *(vg + ag))).hexdigest(),
