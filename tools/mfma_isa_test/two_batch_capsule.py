...
def main() -> int:
    ...
    image = (Path(__file__).parent / "two_batch_mfma.hsaco").read_bytes()
    hip_path = None
    ...
    src_in = torch.zeros(4096, dtype=torch.float32)
    v = src_in.view(torch.int16); v[:] = torch.tensor([0x3c00], dtype=torch.int16)
    src_in = src_in.to("cuda")
    # 传 (in, out) 两参数
    ...
    arg0 = ctypes.c_ulonglong(src_in.data_ptr())
    arg1 = ctypes.c_ulonglong(buf.data_ptr())
    params = (ctypes.c_void_p * 2)(ctypes.cast(ctypes.byref(arg0), ctypes.c_void_p), ctypes.cast(ctypes.byref(arg1), ctypes.c_void_p))
    stream = torch.cuda.current_stream().cuda_stream
    rc = lib.hipModuleLaunchKernel(fn, grid, 1, 1, 64, 1, 1, 4096,
    ...
    result = {
        ...
        "lanevals": vg[:64],
        "first8": vg[:8],
        "non64": sum(1 for v in vg[:64] if v != 64.0),
        "vgpr_all_32": all(v == 64.0 for v in vg[:64]),
        "agpr_all_32": True,
