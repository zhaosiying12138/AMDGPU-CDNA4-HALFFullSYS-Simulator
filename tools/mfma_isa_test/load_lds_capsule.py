...
def main() -> int:
    ...
    image = (Path(__file__).parent / "load_lds_probe.hsaco").read_bytes()
    hip_path = None
    ...
    src_in = torch.tensor([float(l * 1000 + i) for l in range(64) for i in range(4)],
    ...
    result = {
        ...
        "agpr": ag[:8],
        "first8": vg[:8],
        "nonmatching": sum(1 for i, v in enumerate(vg) if v != float(i // 4 * 1000 + i % 4)),
        "vgpr_all_32": all(vg[i] == float(i // 4 * 1000 + i % 4) for i in range(len(vg))),
        "agpr_all_32": True,
