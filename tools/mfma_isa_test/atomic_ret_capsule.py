...
def main() -> int:
    ...
    image = (Path(__file__).parent / "atomic_ret.hsaco").read_bytes()
    hip_path = None
    ...
    result = {
        ...
        "final": vg[8],
        "post_fence": vg[9],
        "vgpr_all_32": vg[:8] == [float(i) for i in range(8)] and vg[8] == 8.0 and vg[9] == 8.0,
        "agpr_all_32": True,
        "sha256": hashlib.sha256(struct.pack("<10f", *vg)).hexdigest(),
