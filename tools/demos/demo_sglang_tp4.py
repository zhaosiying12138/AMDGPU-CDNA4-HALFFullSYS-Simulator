#!/usr/bin/env python3
"""一键演示：SGLang TP4 · Qwen3.5-9B · decode 1 token（9B 的数学最大并行）。

用法：
    python demo_sglang_tp4.py [prompt] [--max-tokens N]

四 gem5 实例（rank 0-3，NCCL Socket）；环境与验收 lane（zcode-sglang-9b-tp4-v3）一致。
"""
...
engine = Engine(
    ...
    context_length=512, chunked_prefill_size=-1,
...
def main() -> int:
    ...
    state = "/tmp/amdgpu-sim-demo-sglang-tp4-9b"
    prepare_state(state, TAG)
    env = build_env(TAG, TP, args.prompt, args.max_tokens,
                    MODELS["9B"], "sglang", state)
    install_shim(state, env)

    banner("AMDGPU-CDNA4-SIM · Qwen3.5-9B · SGLang TP4 · 4× gem5 实例", 58)
    print(f"  {c('模型', 'dim')}: Qwen3.5-9B（36 层，19.3 GB）")
    print(f"  {c('Prompt', 'dim')}: {args.prompt[:44]}…")
    print(f"  {c('并行', 'dim')}: TP4（rank 0-3 各持一个模拟 gfx950，NCCL Socket）")
