#!/usr/bin/env python3
"""一键演示：SGLang TP1 · Qwen3.5-0.8B · decode 1 token（默认）。

用法：
    python demo_sglang_tp1.py [prompt] [--max-tokens N]

单 gem5 实例；引擎环境与验收 lane（zcode-tp1-formal-v1）一致。
"""
...
emit({"event": "log", "text": "构造 Engine(tp=1)……权重经 DTIF fast-copy 注入模拟 gfx950"})
engine = Engine(
    model_path=model, tp_size=1, dtype="bfloat16",
