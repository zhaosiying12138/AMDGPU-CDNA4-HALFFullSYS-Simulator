...
2. **vLLM 0.8B chunked-prefill shape defect (FIXED, gem5 `62d197403`)**: the root cause was four independent gem5 defects stacking on the skinny split-K GEMM path (`wvSplitKrc`, enabled only when m%16==0) — the AGPR alias window was never part of the wave's VGPR reservation, the kernel descriptor's `accum_offset` is not a unified-file alias base, MAI acc-operand encodings (`acc[n]` = `REG_VGPR_MIN+n`) were misread, and `v_dot2c_f32_f16` was unimplemented. After the fix vLLM's own `wvSplitKrc` wrapper passes bit-close (zero NaN) at 16x64x512 for both CuCount=1 and the full CuCount=256 grid; the probe family and the full-LDS regression stay green (`tools/mfma_isa_test/`). The `SAGR_VLLM_CONTEXT_LENGTH` knob is kept.
