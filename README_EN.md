...
| SGLang TP1 / TP2 · Qwen3.5-0.8B (1-token golden `[27841]`; TP2 also has a 10-token gate) | `scripts/test_qwen35_tp.sh 0.8b-tp2`; archived lanes (one lane = one controlled, isolated engine run with its logs/metrics/gate) all PASS |
...
Layer contributions (load path): DTIF (Direct Transport Interface, ROCr's host-side fast-copy channel) fast copy 35.1% > functional-fast 28.4% > KMT mapping cache 26.2% > hybrid CTA 9.3% > idle park/progress 0.9%. Methodology, ablation ladder, and per-layer data: `docs/blog/2026-09-amdgpu-cdna4-halffullsys/` (data sources `data/*.json`).
