...
1. **KMT scratch-admission race (gem5 `61196d8cb`)**: the root cause was gem5 asserting exclusive observation of the host-owned inactive-signal mailbox (publish the code, then require the readback to still hold it) — on slow hosts ROCr's busy-polling handler consumed the request, installed scratch and stored 0 inside the write-to-readback window, which admission misread as a delivery failure before closing the client. Admission no longer interprets mailbox values (readback is a reachability probe only, publication is idempotent, unmet scratch defers unconditionally; teardown arrives via the KMT destroy path); the regression trio is green on the patched build. Forensics: `artifacts/blog-perf-2026-09/results/L0-attempt2-stall-forensics/`.
2. **vLLM 0.8B chunked-prefill shape defect (gem5 `62d197403` + `96089d0f3`)**: the root cause was five independent gem5 defects stacking on the skinny split-K GEMM path (`wvSplitKrc`, enabled only when m%16==0) — (1) the SGPR allocation granularity for gfx942/950 descriptors is 16 SGPRs and the old code halved it (under-reservation: panics or sibling-wave SGPR overlap); (2) the AGPR alias window was never part of the wave's VGPR reservation; (3) the kernel descriptor's `accum_offset` is not a unified-file alias base (the alias base is now derived from the wave's own VGPR region, with the AGPR demand recovered from the code object metadata); (4) MAI acc-operand encodings (`acc[n]` = `REG_VGPR_MIN+n`) were misread and `v_dot2c_f32_f16` was unimplemented; (5) the SGPR admission check missed the SIMD multiplier (a 16-wave workgroup x 208 SGPRs was compared against one SIMD's 2048 although waves spread over all four SIMD files). After the fixes vLLM's own `wvSplitKrc` wrapper passes bit-close (zero NaN) at 16x64x512 for both CuCount=1 and the full CuCount=256 grid, **and the ctx16 engine lane passes its token gate bit-exact end to end** (EV-zcode-0168); the probe family and the full-LDS regression stay green (`tools/mfma_isa_test/`). The `SAGR_VLLM_CONTEXT_LENGTH` knob is kept.
3. **Recent semantic fixes** (gem5 `233dc032a` / runtime `bc6f497`; details in blog §10.7): the hybrid screen now fails closed on LDS and wave-slot demands (large-group-segment kernels fall back to the timing path instead of panicking); s_barrier also waits for the participating waves' outstanding LDS traffic (SI/CI semantics, accepted by the full-LDS serialization capsule); `SAGR_MANAGED_STARTUP_TIMEOUT_MS` widens the managed-session startup window on slow hosts. The correctness suites support offloading to a slow machine — cross-host bit-identical results (operator suite and the 0.8B engine grid) have been verified this way.
