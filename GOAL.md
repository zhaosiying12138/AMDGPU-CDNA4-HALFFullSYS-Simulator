...
**Current state:** `P1-SIGNAL-01-signal-event-accepted`
**Current phase:** `P1`
...
根据 current 指向的 CP-0007 唯一 next_action，从 P1-DISPATCH-01 继续；不要
重做 bootstrap、source freeze、authored runtime baseline 或已通过的 host
transport handshake/queue-control/memory/signal gates，也不要修改已冻结的
SOURCE_LOCK.json 或已登记的 PROJECT_LANES baseline。先在 gem5 与
self-amdgpu-runtime 两个 child 中绑定 functional allocation 到 gem5 GPU VA，
执行 pinned trivial gfx950 AQL dispatch，并用 CP7 signal wait、D2H 和 trace
验证；不要把该 bounded fixture 提升为通用 code-object、ROCr、HIP 或 P2
能力。
